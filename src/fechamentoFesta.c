...
#include "../include/vendaProduto.h" // Certifique-se de que o nome é este mesmo
...
void emitirRelatorioFechamento(Pessoa* lista) {
    ...
    printf("--- CAIXA E BILHETERIA ---\n"); // Corrigido erro de digitação "CASSA"
    printf("Ingressos Vendidos: %d / %d\n", ingressos_vendidos, MAX_INGRESSOS);
    ...
    printf("Refrigerantes:          %d\n", estoque_festa.refrigerantes);
    printf("Sucos restantes:        %d\n", estoque_festa.sucos);
    printf("Aguas restantes:        %d\n", estoque_festa.aguas);
    printf("==================================================\n\n");

    // Salva o estado final de volta no arquivo para garantir consistência
...
int fechamentoFesta() {
    ...
    Pessoa* lista_final = carregarParticipantesFechamento("./data/ingressos/ingressosVendidos.csv");
    ...
    // Limpa a memória da lista temporária utilizada no fechamento
